...
/* ==========================================================
   GlowWise · Glow Types overview
   - No sidebar (uses AppLayout for nav)
   - Mobile responsive
   - Updated Orbit logo
...
const TypeCard = ({ type, isOpen, onToggle, index, isFocused }) => {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onToggle}
        style={{ all: "unset", display: "block", width: "100%", padding: "28px 30px 24px", cursor: "pointer", boxSizing: "border-box" }}
      >
        ...
        <h3 style={{ ...display(28), margin: "0 0 16px", color: type.accent, fontWeight: 500, lineHeight: 1.12 }}>
          {type.name}
        </h3>
        <p style={{ ...bodyText(14), margin: "0 0 20px", fontFamily: FF_DISPLAY, fontStyle: "italic", fontSize: 18, lineHeight: 1.55, color: type.accent, maxWidth: 360 }}>
          "{type.tagline}"
        </p>
        <p style={{ ...bodyText(13.5), margin: "0 0 4px", lineHeight: 1.8, maxWidth: 390 }}>
          {type.essence}
        </p>
        <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "0 30px 30px" }}>
          ...
          <div style={{ padding: "18px 20px", borderRadius: 16, background: "rgba(255,255,255,0.55)", border: `1px solid ${type.color}33` }}>
            ...
            <div style={{ fontFamily: FF_DISPLAY, fontSize: 18, color: C.ink, lineHeight: 1.4, letterSpacing: "-0.01em" }}>
              {type.ritual}
            </div>
          </div>
        </div>
...
export default function GlowTypes() {
  ...
  const scrollToTypes = () => {
    document.getElementById('types-grid')?.scrollIntoView({ behavior: 'smooth' });
  };
    ...
    <AppLayout>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .gt-hero-title { font-size: 36px !important; }
          ...
          .gt-journey-grid { grid-template-columns: 1fr !important; }
          .gt-closing-pad { padding: 36px 24px !important; }
          .gt-closing-title { font-size: 28px !important; }
          .gt-topbar-pad { padding: 16px 18px !important; }
          .gt-card-inner { grid-template-columns: 1fr !important; }
        }
        @media (max-width: 480px) {
          .gt-hero-title { font-size: 30px !important; }
        }
      `}</style>
      ...
      <div className="gt-main-pad" style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 48px 96px" }}>
            ...
            <div className="gt-hero-stats" style={{ display: "flex", gap: 28, flexWrap: "wrap", alignItems: "center" }}>
              {[
                ...
                <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                  ...
                  <span style={{ ...eyebrow(C.mute), fontSize: 10.5, maxWidth: 100, lineHeight: 1.4, letterSpacing: "0.14em" }}>{s.l}</span>
        ...
        <section className="fade-up d1" style={{ marginBottom: 80 }}>
          ...
          <div className="gt-principles-grid" style={{ display: "grid", gap: 20, gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))" }}>
            {[
              ...
              <div key={i} className="fade-up hover-lift" style={{ padding: 30, borderRadius: 22, background: C.paper, border: `1px solid ${C.lineSoft}`, boxShadow: "0 8px 24px -18px rgba(61,74,82,0.18)", animationDelay: `${0.08 * i}s` }}>
        ...
        {/* ── Types Grid ────────────────────────────────── */}
        <section className="fade-up d2" id="types-grid" style={{ marginBottom: 80 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, marginBottom: 28, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 16px", borderRadius: 999, background: C.paperWarm, border: `1px solid ${C.lineSoft}` }}>
              ...
              <span style={{ fontFamily: FF_UI, fontSize: 12, fontWeight: 600, color: C.body }}>
          ...
          <div className="gt-closing-pad" style={{
            ...
            <div style={{ position: "relative", zIndex: 2, maxWidth: 640, margin: "0 auto" }}>
              <div style={{ ...eyebrow(C.sageDark), marginBottom: 20 }}>
                Your rhythm becomes <em style={{ fontStyle: "italic" }}>clearer with time</em>.
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  background: C.sage, color: C.paper, border: "none", cursor: "pointer",
                  fontFamily: FF_UI, fontSize: 14, fontWeight: 600, letterSpacing: "0.01em",
...
// ─── FAQ block (extracted for cleanliness) ───────────────
function FaqBlock() {
  const [open, setOpen] = useState(0);
  const items = [
    { q: "Can I be more than one type?", a: "Yes — most people are a blend, with one type leading. Your type is your centre of gravity, not a cage. GlowWise will show you the primary and any secondary leanings." },
    { q: "What if my type changes?", a: "It happens. Life seasons shift — a new baby, a new job, a different sleep schedule. Your type recalibrates with you. Think of it as a snapshot of how you're functioning now." },
    { q: "Is one type better than another?", a: "No. Every type has its own strengths and its own depletions. The 'best' type is the one that fits you, used with awareness." },
    { q: "What if I don't see myself in any of these?", a: "Worth telling us. Check in for a couple of weeks anyway — most people land in one of the six once they've logged honest data. If you still don't fit, your feedback shapes the next version of the framework." },
  ];
  return (
    <div style={{ background: "#FAF8F5", border: "1px solid rgba(168,153,104,0.10)", borderRadius: 22, overflow: "hidden", boxShadow: "0 10px 30px -22px rgba(61,74,82,0.18)" }}>
      {items.map((it, i) => {
        const isOpen = open === i;
        return (
          <div key={i} style={{ borderBottom: i < items.length - 1 ? "1px solid rgba(168,153,104,0.10)" : "none" }}>
            <button
              onClick={() => setOpen(isOpen ? -1 : i)}
              style={{ all: "unset", display: "flex", width: "100%", padding: "22px 28px", cursor: "pointer", boxSizing: "border-box", justifyContent: "space-between", alignItems: "center", gap: 16 }}
            >
              <span style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 20, color: "#3D4A52", fontWeight: 500, letterSpacing: "-0.01em", lineHeight: 1.3 }}>{it.q}</span>
              <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: "50%", background: isOpen ? "#EDF4EF" : "#F3EFE8", color: isOpen ? "#557E64" : "#A89968", flexShrink: 0, transition: "all 0.3s ease", transform: isOpen ? "rotate(180deg)" : "rotate(0)" }}>
            ...
            <div style={{ maxHeight: isOpen ? 200 : 0, opacity: isOpen ? 1 : 0, overflow: "hidden", transition: "max-height 0.4s cubic-bezier(0.16,1,0.3,1), opacity 0.3s ease" }}>
              <p style={{ fontFamily: "'Manrope', system-ui, sans-serif", fontSize: 14.5, lineHeight: 1.75, color: "#5A6770", margin: 0, padding: "0 28px 24px", maxWidth: 720 }}>{it.a}</p>
            </div>
          </div>
        );
      })}
